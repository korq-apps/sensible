...
release_guard_assets="$(cat <<'EOF'
          expected_assets=(
            "sensible-gnome-debian-testing-amd64.iso.torrent"
            "sensible-gnome-debian-testing-amd64.iso.magnet.txt"
            "sensible-gnome-debian-testing-amd64.iso.sha256"
            "sensible-kde-debian-testing-amd64.iso.torrent"
            "sensible-kde-debian-testing-amd64.iso.magnet.txt"
...
assert_contains "release guard enumerates all six promised metadata assets" "$workflow_source" "$release_guard_assets"
assert_contains "release guard rejects missing or empty assets" "$workflow_source" 'if [ ! -s "$asset" ]; then'
assert_contains "release guard validates checksum filenames" "$workflow_source" '[[ "$filename" != "$iso" ]]'
...
for variant in gnome kde; do
    ...
    printf 'fixture torrent\n' > "${release_fixture}/${iso}.torrent"
    printf 'magnet:?xt=urn:btih:fixture\n' > "${release_fixture}/${iso}.magnet.txt"
...
release_guard_output="$(cd "$release_fixture" && bash -c "$release_guard_script" 2>&1)"
release_guard_rc=$?
assert_rc "release guard rejects a malformed checksum" 1 "$release_guard_rc"
assert_contains "checksum failure identifies the invalid checksum asset" \
    "$release_guard_output" "Invalid checksum record"
