...
| `biometrics_test.sh` | V4L2 discovery, stable links, ambiguous/inaccessible cameras, fprintd/PAM inventory; standalone configuration/rollback, symlink and concurrent-write rejection, temporary PAM cleanup on failure/timeout/cancel, package hash/identity checks; guided activation, watchdog/boot rollback, policy rejection and real Linux-PAM control-flow tests against temporary services; wizard prompts and the branded probe/wizard presentation (plain when piped or `NO_COLOR`, colour adds only escape codes, aligned step boxes, safe logo loading); packaged-install versus checkout behaviour; image staging (`stage-biometrics.sh`: build-manifest, identity and pin checks, model pins, tool tree, launcher, provenance) and the `0280-biometrics` build hook against fixture roots; fixtures do not establish live desktop authentication or a chroot install |
...
- **Howdy-next package build and image acceptance**: the container build,
  the `apt-get --simulate` reuse decision and Howdy's offline acceptance of the
  baked models run only inside the ISO build; the fixtures use tool doubles.
