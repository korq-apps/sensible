...
# Locate an OVMF code+vars pair. Prefer the 4M split build (Debian/Ubuntu);
# use a plain, non-secboot code image so the ISO boots with Secure Boot off.
OVMF_CODE="" OVMF_VARS=""
for pair in \
...
if [ -z "${OVMF_CODE}" ]; then
    echo "Error: no OVMF code/vars pair found. Install 'ovmf' (Debian/Ubuntu) or 'edk2-ovmf' (Arch)." >&2
...
echo "==> Booting ${ISO_PATH} (timeout ${SMOKE_TIMEOUT}s, serial -> ${SERIAL_LOG})"
set +e
